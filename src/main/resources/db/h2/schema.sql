create table `camera` (
    `camera_id` int unsigned not null auto_increment comment '相机ID',
    `camera_name` varchar(255) not null default '' comment '相机名称，具有唯一性',
    `ax_camera_number` int unsigned not null comment '摄像头在爱信系统中的编码',
...
create table `device` (
    `device_id` int unsigned not null auto_increment comment 'IOT设备ID',
    `device_name` varchar(255) not null default '' comment 'IOT设备名称，具有唯一性',
    `status` int unsigned not null default 0 comment 'IOT设备状态：0-正常，1-断线',
...
create table `device_camera` (
    ...
    `device_id` int not null comment '设备表ID',
    `camera_id` int not null comment '相机表ID',
    `ai_type_array` varchar(255) not null default '' comment 'AI检测算法类型数组，使用逗号分隔',
    primary key (`id`)
) engine = innodb default charset = utf8mb4;

create table `person` (
    `person_id` int unsigned not null auto_increment comment '人员ID',
...
create unique index `uk_person_person_name` on `person` (`person_name`);

create table `device_person` (
    `id` char(36) not null default '' comment '设备与人员联合ID，使用36位UUID',
    `device_id` int not null comment '设备表ID',
    `person_id` int not null comment '人员表ID',
    primary key (`id`)
) engine = innodb default charset = utf8mb4;
...
create table `event` (
    ...
    `event_type_value` int not null comment '预定义事件值，与事件类型有关',
    `event_time` datetime not null comment '事件发生的时间',
    `description` varchar(200) default null comment '事件描述',
